    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-12 max-w-full overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground">{t.bakiDesc}</p>
        </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          <Badge variant="secondary" className="text-[8px] h-4 px-1">Qty: {record.quantity}</Badge>
                          <span className="text-[8px] text-muted-foreground">{new Date(record.takenDate).toLocaleDateString()}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-black text-sm text-primary">{currency}{record.amount.toLocaleString()}</p>
                        {record.status === 'paid' && <span className="text-[8px] text-green-600 font-bold uppercase">Paid</span>}
                      </div>
                    </div>
                    {record.status !== 'paid' && (
                      <div className="mt-3 flex justify-end">
                         <Button size="sm" variant="outline" className="h-7 text-[9px] font-black uppercase text-accent border-accent/30" onClick={() => actions.payBakiRecord(detailsCustomer.id, record.id, record.amount - (record.paidAmount || 0), record)}>
                           Mark Paid
                         </Button>
                      </div>
                    )}
      ...
      <Dialog open={isAddOpen} onOpenChange={handleOpenAddDialog}>
        <DialogContent className="w-[95vw] sm:max-w-[500px]">
            ...
            {addStep === 1 ? (
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <Input placeholder="Type to search..." value={productSearch} onChange={e => setProductSearch(e.target.value)} className="h-10 border-accent/20" />
                  {filteredProducts.length > 0 && (
                    <Card className="absolute z-50 w-full mt-1 max-h-40 overflow-hidden shadow-xl bg-white">
          ...
          <DialogFooter>
            {addStep === 1 ? (
              <Button className="w-full bg-accent h-12" onClick={() => setAddStep(2)} disabled={!newCustomer.firstName}>Next</Button>
            ) : (
              <Button className="w-full bg-primary h-12" onClick={handleAddCustomerAndBaki}>Save Everything</Button>
            )}
          </DialogFooter>
        </DialogContent>
      </Dialog>
