  ...
  const handleAIDescription = async () => {
    ...
    try {
      const result = await generateProductDescription({
        ...
        targetAudience: "Customers looking for quality eyewear or products."
      })
            ...
            <DialogContent className="sm:max-w-[600px]">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="category" className="col-span-3" value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})} />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid grid-cols-2 items-center gap-2">
                    <Label className="text-right text-xs">Buy Price</Label>
                  ...
                  <div className="grid grid-cols-2 items-center gap-2">
                    <Label className="text-right text-xs">Sell Price</Label>
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="stock" type="number" step="0.01" className="col-span-3" value={newProduct.stock} onChange={e => setNewProduct({...newProduct, stock: e.target.value})} />
                </div>
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label className="text-right mt-2 text-xs">Description</Label>
                  <div className="col-span-3 space-y-2">
                    <Textarea className="min-h-[80px]" value={newProduct.description} onChange={e => setNewProduct({...newProduct, description: e.target.value})} />
                    <Button size="sm" variant="secondary" className="w-full gap-2 text-xs" onClick={handleAIDescription} disabled={isGenerating}>
                      <Sparkles className="w-3.5 h-3.5" /> {isGenerating ? "Perfecting..." : "Generate AI Description"}
                    </Button>
                  </div>
                </div>
              </div>
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem className="gap-2 text-destructive" onClick={() => actions.deleteProduct(p.id)}>
